...
func (server *GxTcpServer) Start(port string) error {
	Debug("lister %s ", port)
	listener, err := net.Listen("tcp", port) //Start listening.
	...
	Debug("server start, host: %s", port) //Output debug message.

	for {
		conn, err1 := listener.Accept() //Accept data.
